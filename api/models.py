...
class User(AbstractUser):
    ...
    def save(self, *args, **kwargs):
        ...
        if not self.last_login:
            ...
            Categories.objects.create(
                categoryName='Внезапная покупка',
                user=self,
                category_type='once',
                income_outcome='outcome'
            )
...
class Categories(models.Model):
    ...
    category_type = models.CharField(
        max_length=10,
        choices=CAT_TYPES,
        default=constant_cat
    )
    income_outcome = models.CharField(
        max_length=11,
        choices=CAT_INCOME_OUTCOME,
        default=income_cat
    )
